...
setwd("C:/Users/uzma.khan/OneDrive - Aga Khan University/Documents/R-IM-URK")  
...
data <- data.table::fread("Mobility-data-Final_1.csv", data.table = FALSE)
...
data$q13a <- as.factor(data$q13a)
levels(data$q13a) <- c("No", "Yes")

data$q13b <- as.factor(data$q13b)
levels(data$q13b) <- c("No", "Yes")
table(data$q13b)

data$q13c <- as.factor(data$q13c)
levels(data$q13c) <- c("No", "Yes")
table(data$q13c)
...
## Univariate Logistic regression of age groups with RTI
table(data$age_cat)
data$age_cat <- (data$age_cat)
labelled::var_label(data$age_cat) <- "Age groups"
model2 <- glm(rti_consult ~ age_cat, family = "binomial", data = data)
pretty_regression_table(model2)

data$age_cat <- relevel(data$age_cat, ref = "15 to 19")
model3 <- glm(rti_consult ~ age_cat, family = "binomial", data = data)
...
model4 <- glm(rti_consult ~ type_school, family = "binomial", data = data)
...
model5 <- glm(rti_consult ~ gender,  family = "binomial", data = data)
...
#Logistic of Cycle variables

model13 <- glm(rti_consult ~ q13a,  family = "binomial", data = data)
pretty_regression_table(model13)

model14 <- glm(rti_consult ~ q13b,  family = "binomial", data = data)
pretty_regression_table(model14)

labelled::var_label(data$q13c) <- "Cycling without grown up"
model15 <- glm(rti_consult ~ q13c,  family = "binomial", data = data)
pretty_regression_table(model15)

data$q13a <- as.character(data$q13a)
data$q13a[is.na(data$q13a)] <- "No"
data$q13a <- as.factor(data$q13a)
levels(data$q13a) <- c("No", "Yes")

data$q13b <- as.character(data$q13b)
data$q13b[is.na(data$q13b)] <- "No"
data$q13b <- as.factor(data$q13b)
levels(data$q13b) <- c("No", "Yes")
table(data$q13b)

data$q13c <- as.character(data$q13c)
data$q13c[is.na(data$q13c)] <- "No"
data$q13c <- as.factor(data$q13c)
levels(data$q13c) <- c("No", "Yes")
table(data$q13c)

model16 <- glm(rti_consult ~ data$q13a,  family = "binomial", data = data)
pretty_regression_table(model16)

model17 <- glm(rti_consult ~ data$q13b,  family = "binomial", data = data)
pretty_regression_table(model17)

model18 <- glm(rti_consult ~ data$q13c,  family = "binomial", data = data)
pretty_regression_table(model18)

#Multivariate Logistic reg
data$any_activity <- relevel(data$any_activity, ref = "No activity on the weekend")
model19 <- glm(rti_consult ~  age + gender + accompaniment_schoolreturn + cross_road + 
                 allow_public_bus + any_activity,
                family = "binomial", data = data)

summary(model19)
or.ci<- exp(cbind(OR = coef(model19), confint(model19)))
round(or.ci, digits=2)


#Multivariate Logistic reg
data$any_activity <- relevel(data$any_activity, ref = "No activity on the weekend")
model20 <- glm(rti_consult ~  age_cat + gender + accompaniment_schoolreturn + cross_road + 
                 allow_public_bus + any_activity,
               family = "binomial", data = data)

summary(model20)
or.ci<- exp(cbind(OR = coef(model20), confint(model20)))
round(or.ci, digits=2)
